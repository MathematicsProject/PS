...
public class AlphaAPISample {
    ...
    // PUT YOUR APPID HERE:
    private static String appid = "EJ6HRW-2GL2G9AJHU";

    public void connetAPI(ArrayList<String> getTitle, ArrayList<String> resultURL, String inputData) {

        // Use "pi" as the default query, or caller can supply it as the lone command-line argument.
        String input = inputData; //input equation
//        if (args.length > 0)
//            input = args[0];
                        ...
                        for (WASubpod subpod : pod.getSubpods()) {
                            for (Object element : subpod.getContents()) {
                                if (element instanceof WAImage) {
                                    System.out.println(((WAImage) element).getURL()); // jsp mapping save url String
                                    resultURL.add(((WAImage) element).getURL());
                                    getTitle.add(pod.getTitle());
                                    System.out.println("");
                                }
                            }
                        }
